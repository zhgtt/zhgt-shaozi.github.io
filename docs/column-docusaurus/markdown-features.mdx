...
- 由于预定义的组件无法在 **服务端** 运行，所以需要使用 Docusaurus 内置的组件 `<BrowserOnly />` 进行包裹，让其只能在客户端(浏览器)中运行，不在服务端运行，如下:
...
```mdx-code-block
<BrowserWindow>
  <img src={require('./img/markdown-features/wallhaven-x8d8gz.jpeg').default} alt="Example Image" />
</BrowserWindow>
```
