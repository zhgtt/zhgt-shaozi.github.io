...
> 本章节学习 Docusaurus 中新增的 Markdown 语法 & 功能
...
- 通过 `代码注释` 来高亮指定的代码或代码区域；


    ```jsx title="多行代码高亮"
    const HighlightSomeText = (highlight) => {
        if (highlight) {
          {*/ highlight-next-line */}   // 在实际代码中，要改成 // highlight-next-line 的形式，这里只是说明一下
          return '高亮的文本内容...';
        }
...
function Clock(props) {
  ...
  return (
    <div>
      <h3 style={{ color: '#C3272B' }}>It is {date.toLocaleTimeString()}</h3>
    </div>
  );
}
```
...
:::caution 离谱小贴士
- 由于 react-live 交互式代码编辑器不支持 **直接引入组件或方法**，因此可以在 **theme** 目录中创建 **ReactLiveScope** 文件(react-live 的作用域)，用来预先定义可用(导出)的组件或方法;
- 创建该文件之后，需要 **重启该项目**（仅需重启一次即可）;
- 导出的组件或方法可以在 react-live 编辑器中直接使用;

```jsx title="src/theme/ReactLiveScope/index.tsx"
import React from 'react';
import { Button, Input } from '@arco-design/web-react';   // 引入组件
...
```mdx-code-block
<BrowserWindow>
  <img src={require('/img/docs/markdown-features/wallhaven-x8d8gz.jpeg').default} alt="Example Image" />
</BrowserWindow>
```

### Docusaurus 的拓展资源

- Ducusaurus 支持直接导入 `svg` 图像，并且将其自动转换为 React 组件:
...
```mdx-code-block
import GodOfWealthSVG from '@site/static/img/docs/markdown-features/caishenye.svg';
...
- Ducusaurus 支持 `主题` 图像，其内置了一个 `ThemedImage` 组件，根据当前主题切换对应的图像:
