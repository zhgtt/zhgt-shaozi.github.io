export const todoListCode = `import React, { useState } from 'react';
  ...
  const handleMove = (_record, index: number, type: 'up' | 'down') => {
    ...
    switch (type) {
      case 'up':
        ...
        // 或 利用 Array.splice 返回被替换的元素的特性，进行位置互换
...
export const todoListEditCode = `import React, { useEffect, useState, useRef, useContext, useCallback } from 'react';
...
export const todoListCssCode = `.topRow {
  > td {
